...
function incrementShortsHiddenCounter() {
  const storage = (typeof browser !== 'undefined' && browser.storage && browser.storage.local) ? browser.storage.local : chrome.storage.local;
  // Always increment by 1, and always show toast if enabled
  if (storage.get.length === 1) {
    // Callback style (chrome)
    storage.get(['shorts_hidden_count'], (result) => {
      const current = result && result.shorts_hidden_count ? result.shorts_hidden_count : 0;
      const next = current + 1;
      storage.set({ shorts_hidden_count: next }, () => {
    ...
    storage.get(['shorts_hidden_count']).then(result => {
      const current = result && result.shorts_hidden_count ? result.shorts_hidden_count : 0;
      const next = current + 1;
      storage.set({ shorts_hidden_count: next }).then(() => {
        if (showCounterToastEnabled) {
          showCounterToast(next);
        }
      });
...
function removeShorts() {
    ...
    if (contents) {
      ...
      reelShelves.forEach(shelf => {
        if (shelf.dataset.noShortsProcessed) return;
        ...
        if (titleSpan && titleSpan.textContent && titleSpan.textContent.toLowerCase().includes('shorts')) {
          shelf.dataset.noShortsProcessed = '1';
          if (shelf.parentNode) {
            if (zapModeEnabled) {
              zapAndRemove(shelf);
            } else {
          ...
          if (span.textContent && span.textContent.toLowerCase().includes('shorts')) {
            if (shelf.parentNode) {
              ...
              gridRows.forEach(row => {
                gridItems = gridItems.concat(Array.from(row.querySelectorAll('div.ytGridShelfViewModelGridShelfItem')));
              });
              if (gridItems.length === 0) {
                // fallback: try to count direct grid items if structure changes
                gridItems = Array.from(shelf.querySelectorAll('div.ytGridShelfViewModelGridShelfItem'));
              }
              if (gridItems.length > 0) {
                gridItems.forEach(item => {
                  if (!item.dataset.noShortsProcessed) {
                    item.dataset.noShortsProcessed = '1';
                    incrementShortsHiddenCounter();
                  }
                });
              } else {
